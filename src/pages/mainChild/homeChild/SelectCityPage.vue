<template>
  <div class="scroll-box">
    <app-head class="header-box" :backBool="true":searchBool="true"></app-head>
...
export default {
    ...
    setScrollPos(ev){//点击改变滚动条的位置
        ...
        for(var i=0;i<userIndexs.length;i++){
            if(userIndexs[i].innerHTML==ev.target.innerHTML){
                var tH=document.getElementsByClassName("header-box")[0].offsetHeight+document.getElementsByClassName("location-box")[0].offsetHeight;
  ...
  created(){
      this.$store.commit("setHeaderTitle","选择城市");
  },
  mounted() {}
};
</script>
